...
"""
# app.py (updated — unique keys to avoid StreamlitDuplicateElementId)
...
def make_key(*parts):
    ...
    return key[:200]  # cap length to be safe
...
with tabs[0]:
    ...
    for res in results:
        ...
        exp_key = make_key(res['file'], res['hdu_index'], 'expander', 'raw')
...
with tabs[1]:
    ...
    for res in results:
        label = f"{res['file']} (HDU {res['hdu_index']})"
        exp_key = make_key(res['file'], res['hdu_index'], 'expander', 'smooth')
        with st.expander(label, expanded=False, key=exp_key):
            wl = res['wl']; fl = res['fl']; err = res['err']
            if raw_only:
...
with tabs[2]:
    ...
    for res in results:
        label = f"{res['file']} (HDU {res['hdu_index']})"
        exp_key = make_key(res['file'], res['hdu_index'], 'expander', 'mol')
        with st.expander(label, expanded=False, key=exp_key):
            wl = res['wl']; fl = res['fl']
            if raw_only:
                fl_proc = fl
...
with tabs[3]:
    ...
    if len(results) < 2 or not stack_enabled:
        ...
        if enable_downloads:
            ...
            dl_key = make_key('stacked','download','csv')
...
with tabs[4]:
    ...
    for r in results:
        ...
        df_key = make_key(r['file'], r['hdu_index'], 'table')
        st.dataframe(df.head(500), use_container_width=True)  # show first 500 rows
        if enable_downloads:
